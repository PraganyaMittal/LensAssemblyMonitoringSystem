/**
 * PCSelectionList - Virtualized, Accessible PC Selection Component
 * 
...
function createVirtualRows(
    ...
    const cardsPerRow = Math.max(
        1,
        Math.floor((containerWidth + pcCard.gap) / (pcCard.minWidth + pcCard.gap))
    );
    ...
    for (const lineNumber of sortedLines) {
        ...
        for (let i = 0; i < linePCs.length; i += cardsPerRow) {
            rows.push({
                ...
                height: pcCard.height + pcCard.gap,
            });
        }
 ...
 */
const PCCard = memo(function PCCard({
    pc,
    onClick,
    isSelected = false,
    tabIndex = 0,
    ...
    const ariaLabel = `PC ${pc.ipAddress}, Line ${pc.line ?? 'unknown'}, ${statusLabel}`;
    ...
    return (
        <motion.button
            ...
            style={{
                ...
                // Dimensions
                minWidth: pcCard.minWidth,
                height: pcCard.height,
                padding: spacing.sm,
...
export default function PCSelectionList({
    pcs,
    onSelectPC,
    loading,
    selectedPcId,
    ...
    const virtualizer = useVirtualizer({
        ...
        estimateSize: (index) => virtualRows[index]?.height ?? pcCard.height,
        overscan: 5,
    });
    ...
    if (loading) {
        return (
            ...
            >
                <LoadingSpinner label="Loading PCs..." />
            </article>
        );
    }
    ...
    if (pcs.length === 0) {
        return (
            ...
            >
                <EmptyState
                    icon={<Server size={32} />}
                    message="No PCs available"
                    description="Connect PCs to the factory network to see them here."
                />
            </article>
        );
    }
                                ...
                                >
                                    {row.pcs?.map((pc) => (
                                        <PCCard
                                            key={pc.pcId}
                                            pc={pc}
                                            isSelected={pc.pcId === selectedPcId}
