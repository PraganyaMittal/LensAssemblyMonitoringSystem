...
// --- Merged from ConfigService ---

bool ConfigManager::UploadConfigToServer(const std::string& configFilePath, RestClient* client, const std::string& requestId) {
    ...
    json response;
    return client->Post(AgentConstants::ENDPOINT_UPLOAD_CONFIG, request, response);
}

bool ConfigManager::ApplyConfigFromServer(const std::string& filePath, const std::string& content) {
    if (content.empty()) {
        return false;
    }
    return WriteConfigFile(filePath, content);
}